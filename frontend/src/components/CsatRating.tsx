...
export function CsatRating({
  ...
  if (existingRating) {
    return (
      <div className="bg-white border border-slate-200 rounded-lg p-5">
        <h2 className="text-sm font-semibold text-slate-700 mb-2">Rating Kamu</h2>
        <div className="flex gap-1 text-xl">
          {[1, 2, 3, 4, 5].map((star) => (
            <span key={star} className={star <= existingRating ? 'text-yellow-400' : 'text-slate-200'}>
              ★
            </span>
          ))}
        </div>
        {existingComment && (
          <p className="text-sm text-slate-600 mt-2 italic">"{existingComment}"</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg p-5">
      <h2 className="text-sm font-semibold text-slate-700 mb-1">Puas dengan penanganan ticket ini?</h2>
      <p className="text-xs text-slate-500 mb-3">Kasih rating buat bantu kami tingkatkan layanan.</p>
